...
```{r}
model <- lm(volume~p95+I(p95^2),data=sampled)
summary(model)
...
for(i in 1:repeats){
  ...
  result_reg[cont,"rep"]<-i
  #
  # Store the values of the mean for the forest (not the estimated mean)
...
These results look decent. Now we look at the model predicted values of the repetitions.
...
Check the true model for volume and how the data was actually generated.
...
example3$volume <- ifelse(sample(1600,20)%in%example1$id,example1$volume,NA)


```

Based on the true data generating process, we would expect differences of around 100 between stands. But why are we getting values that are almost the same for all stands and for the foerst. The model has to be failing in some way. Even worse, we have analyzed the uncertainty based on the assumption that the model holds but it seems that the model we are fitting does not hold. Can we trust this model? Are we missing something?
