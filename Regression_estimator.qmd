...
`eblupBHF` provides estimates for stands and the fitted model using the 4 inputs listed above. `pbmseBHF` estimates the mse using parametric bootstrap, it requires an aditional input
...
[**Step 2. Run best subsets (add vif and initialize to NA and sort by adjusted r-squared)**]{.underline}
...
[**Step 3. Tag models with large vif and test if error variance increases.**]{.underline}
