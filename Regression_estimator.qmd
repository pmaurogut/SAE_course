...
Let's now plot our results as a function of the sample size.
...
2.  As we know the sample mean is unbiased and, on average, tends to coincide with the true means for the forest and the stands.
...
Let's now compare the sample mean to the regression estimator. We will simulate a precission requierement of +- 10% in our estimates ploting discontinuous lines around the true value
...
-   For the entire forest the rergression estimator does much better for all sample sizes. This is because the relationship between volume and p95 can be approximated well by a linear function. The relathionship, however, is not linear, it was created with the code snipet below, but this method does not rely on assuming that a linear relationship exists.
...
Take the code snippet above and replace the #PLACEHOLDER comment above with data generated by you. You can make the data have a more or less linear relationship. Then re-run the code and see what happens.
