...
Check the true model for volume and how the data was actually generated and plot observations from different stands with different colours. Pay attention to the magnitude of the quantities in stand_effects.
...
1.  Use regession estimators for each stand. But we have about 3 plots per stand and chances are that we have stands with no plots. In the best case, regression estimators for the stand will have a large variance.

2.  We could change the model so it includes a different intercept for each stand. But in this model the intercepts for the stands can take any value (we are not assumming anything about how they are generated) and to estimate them we have only about 3 plots per stand so our estimates will also have a large variability.
