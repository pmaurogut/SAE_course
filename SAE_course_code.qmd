...
Let's now plot our results as a function of the sample size. We will simulate a precision requirement of +- 10% in our estimates plotting discontinuous lines around the true value as references.
...
$$
\bar{Vol} \sim N(\frac{1}{1600}\sum_{i=1}^{1600}(25+6P95_i),\frac{1}{1600}130^2)= N(328.4025,3.25^2)
$$
...
We can see that assuming independence is a very strong assumption. With independence there is a large compensation of departures from the mean. Some units deviate above the mean and some units deviate below the mean and positive an negative deviations occur independent of each other. This causes that the variance of the mean for the forest is very small, there is still some variance, which is important, and tells us that in a model-sbased etting the target parameter is a random variable, but with small variance. We will see that the independence assumption is not realistic in many situations. Stands are differentiated units, someone draw the stand boundaries for a reason, and it is reasonable to expect some within stand correlation. For example, we can expect errors for volume for units within a give stand to show some correlation.
...
To assess the uncertainty of our estimate we consider alternative values, compatible with the model, for the sampled and unsampled parts. We will work with a sample of 50 units which implies \~3 plots per stand. We will use parametric bootstraping and repeat the process of fitting the model and computing predictions for all population units 1000 times. However, there are some differences with respect to bootstrapping for design-based techniques and for the simulation used for the regression estimator in the previous section:
...
3.  The mean volume for the forest and the stands are random variables themselves. They are not fixed quantities that we can take as a reference, therefore, we will generate the true value for the forest and each stand in every iteration using the model.

4.  As mean volume for the forest and the stands are random variables, we cannot use them as a reference because they change in every iteration. Thus, we will measure the differences between the mean values for the forest and each stand and the predicted values obtained in each iteration.
...
for(i in 1:B){
  # create a copy of sampled and unsampled for convenience
  sampled_i <- sampled
...
These results look decent. Now we look at the predicted values for all repetitions for stands 1 and 2.

```{r}
par(mfrow=c(2,1))
hist(result_reg$pred_mean_stand1,main="predicted values stand 1")
hist(result_reg$pred_mean_stand2,main="predicted values stand 2")
hist(result_reg$pred_mean_forest,main="predicted values forest")
```
...
Based on the true data generating process, we would expect differences of around 220 between stands 1 and 2. Why are we getting values that are almost the same for all stands and for the forest? The model has to be failing in some way. Even worse, we have analyzed the uncertainty based on the assumption that the model holds but it seems that the model we are fitting does not hold. Can we trust this model? Are we missing something?

**Our model does not account for stand effects that actually exist**

[**We are not modeling stand effects. Our assumption of a pure linear relationship is wrong.**]{.underline}

[**This type of modeling (assumption) is called synthetic, it assumes that a general model holds for all stands, but that might be wrong. That false assumption has important risks if stands are actually differentiated units. And chances are that there some differences, at the end, if someone draws a stand boundary it is probably because something looked different from the surrounding area.**]{.underline}
...
2.  We could change the model so it includes a different intercept for each stand as a fixed effect (model parameter).
...
There are several packages that allow us working with small area models. Below is a description of the main packages that we can use.
...
-   Provides a `predict` function which allows generating raster layers using the `terra` package.
...
-   Once you obtain the model parameters, you have compute stand level estimates and uncertainty metrics yourself (could be hard). To do this, the best reference is the Small Area Estimation book from Rao and Molina (2015).

## Example unit-level

To work with unit-level models we need plots with coordinates that we use to extract their auxiliary information. In this case, we have already extracted two auxiliary variables from lidar data. The lidar data was collected in 2009 and plots were measured in 2010. The auxiliary variables are the 95th percentile of the heights of lidar (P95) returns and the standard deviation of those heights (SD_H).
...
par(mfrow=c(2,2))
plot(population_aux_info[["P95"]]) 
plot(population_aux_info[["SD_H"]]) 
plot(population_aux_info[["ID_SMA"]])
plot(plots[,"ID_SMA"],add=TRUE,pch=20,col="red",cex=0.5)  
```
...
We could aggregate these predictions using zonal stats, however, this approach does not let us get unbiased estimates of the mean squared error (mse). To get that it would be necessary to use the model coefficients and implement the mse estimators ourselves.

### Using the sae package

To get point estimates (eblups) of QMD for stands and their associated uncertainty we can use the `eblupBHF` and `pbmseBHF` functions in the `sae` package. These functions the following inputs
...
**IMPORTANT** To get 3) and 4) we need access to the auxiliary information for the entire population. That information is in the raster file "Examples_Unit_Level.tif". The first band contains P95, the second contains SD_H and the third the stand IDs.
...
Once we have 3) and 4) we can use the `eblupBHF` in the `sae` package, attach the plots object so `eblupBHF` will find the information it needs. `eblupBHF` obtain point estimates using the basic unit level model . **Warning:** **Stands without samples have to have NAs in the response column** **of the training data**.
...
The element eblup stores the stand level estimates.
...
To generate outputs that we can share with gis users we are going to merge all results in a data.frame. **Be aware that in the results the stand identifiers are renamed to "domain".**

```{r}
eblups <- result$est$eblup
mses <- result$mse
...
We can compare mean square errors (mses) of direct estimates and eblups as a function of the small area sample size. In the figure, Direct is the sample mean and EBLUP the prediction with the unit-level model.
...
plot(lidar[["zq95"]])
plot(stands,add=TRUE,col=rgb(1,0,0,0.25))
plots
```
...
We recommend doing the last step manually or at least using some graphical summaries to assist the process. Step 3 is the step in which we will check our model, remember that these techniques are based on the model, i.e., the model must hold, so it is important to be patient and thorough in this step.

```{r}
plots<- st_read("BMEF.gpkg",layer="plots")
stands <- st_read("BMEF.gpkg",layer="stands")
lidar <- rast("BMEF_Metrics.tif")
# mapview(lidar[["zq95"]]) + mapview(stands) + mapview(plots)
plot(lidar[["zq95"]])
plot(stands[,1],col=rgb(1,0,0,0.25),add=TRUE) 
plot(plots[,1],add=TRUE,pch=20,col="blue")
```

[**Step 1. Removal of highly correlated variables and variables with no variation**]{.underline}

```{r}
# lets remove the geometry of the spatial object to make it a plain data frame (data)
data <- st_drop_geometry(plots) 
predictors <- c(10:62)
X<-data[,predictors]
...
n <- rowSums(summary_select$which) - 1
adjr2 <- summary_select$adjr2
plot(n,adjr2,ylim=c(0,1))
```
...
adjr2 <-adjr2[order_model]
models_list <- models_list[order_model]
```

[**Step 3-b. Sort by adjusted r-squared and start testing manually.**]{.underline}

```{r}
models_list <- models_list[order(adjr2,decreasing=TRUE)]
selected <- models_list[[1]]$model
summary(selected)
```

We have to include the stand effects.

```{r}
selected_mixed <- lme(VOL~zmax+ zq50+ zq65 + itot + ikurt+ p4th,
                      random=~1|StandID,data  = data)
summary(selected_mixed)
```

If we want to test the next model in the list, more parsimonious

```{r}
second <- models_list[[2]]$model
summary(second)
```

```{r}
selected_mixed2 <- lme(VOL~zmax + itot + ikurt + ipcumzq50 + p4th,
                      random=~1|StandID,data  = data)
summary(selected_mixed2)
```

continue inspecting the list until you find a suitable model
